...
class TypedSid(StringSid):
    ...
    - TypedSid: has a type and a fields dictionary, may contain meta-characters that can resolve to multiple Sids
    - ConcreteSid: describes one single data, that can exist or not
    - DataSid: a sid that may call data sources internally
    - Nice idea >>> Sid('hamlet', 'a', 'prop') -> Sid('hamlet/a/prop')
...
class DataSid(PathSid):
    ...
    def get_last(self, key: Optional[str] = None) -> Sid:
        ...
        found = DS().find_one(self.get_with(key=key, value='>'), as_sid=True)
