...
from spil.sid.search.transformers.typed_narrow import execute as narrow
from spil.sid.search.transformers.extrapolate import execute as extrapolate

# transformers by execution order for existing searches
list_search_transformers = [extensions, or_op, expand, narrow]
...
def transform(sid, transformers):
    """
    Takes the given sid, executes Search Sid transformers and returns a sorted list of unique search sids.
...
def unfold_search(search_sid, do_uniquify=False, do_extrapolate=False):
    ...
    # transforming
    search_sids = transform(
        str(search_sid), list_search_transformers + ([extrapolate] if do_extrapolate else [])
    )
