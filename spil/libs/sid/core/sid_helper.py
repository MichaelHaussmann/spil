...
def compare_by_template(a, b, template=None):
    ...
    if template:
        ...
        if not set([a, b]) <= set(template):
            warn('Not all of "{}" and "{}" are included in template "{}". Returning string order.'.format(a, b, str(template)))
            template = []
        for item in template:
            ...
            if item == b:
                return 1
        if template:
            warn('Unable to order "{}" vs "{}" (using "{}"). Returning string order.'.format(a, b, template))
    return 1 if a > b else -1
